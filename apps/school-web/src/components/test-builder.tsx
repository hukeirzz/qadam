...
export function TestBuilder({
  schoolId, topics, classes, students, topicStats, onDone, onCancel,
}: {
  schoolId: string;
  ...
  const classStudents = selClass === ALL_CLASSES ? students : students.filter((s) => s.class_id === selClass);
  ...
  async function handleSend() {
    ...
    const { error: saveErr } = await api.staffData.createSchoolTest(
      ...
      sess.user?.id ?? '',
      {
        title: subjectName ? `${subjectName}: ${topicTitle}` : topicTitle,
        targetRank: rank,
  ...
  function reset() {
    setStep(1); setQuestions([]); setSelStudents(new Set()); setError(null);
  }
        ...
        <Card className="space-y-5">
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={generate} disabled={generating || !topicId}
              className="flex items-center justify-center gap-2 rounded-xl bg-brand px-5 py-3 text-sm font-semibold text-white transition hover:bg-brand-hover disabled:opacity-60">
              <SparkleIcon className="h-5 w-5" />
              {generating ? 'Генерируем тест…' : 'Сгенерировать тест'}
            </button>
          </div>
          <p className="text-center text-xs text-slate-400">Создаются заготовки вопросов, которые нужно отредактировать на следующем шаге.</p>
        </Card>
      )}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <select value={selClass} onChange={(e) => setSelClass(e.target.value)}
                  ...
                  <option value={ALL_CLASSES}>Все классы</option>
      ...
      {step === 4 && (
        <Card className="flex flex-col items-center gap-4 py-12 text-center">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={onDone} className="rounded-xl px-5 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-100">К списку тестов</button>
          </div>
        </Card>
      )}
    </div>
